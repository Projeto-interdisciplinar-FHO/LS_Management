  ...
  <div class="taxonomy-page">
    <header class="page-header">
      <div class="header-copy">
        ...
        <p>Lista das raças registradas e a espécie a que cada uma pertence.</p>
      </div>
      <button @click="openModal" class="btn-primary">+ Adicionar raça</button>
    </header>
    ...
    <section class="content-card">
      <div class="section-header">
        <div>
          <h2>Raças</h2>
          <p>Consulte o cadastro técnico das raças já registradas.</p>
        </div>
        <span class="count-badge">{{ breeds.length }} cadastradas</span>
      </div>
      ...
      <div v-else-if="breeds.length === 0" class="empty-state">
        <p>Nenhuma raça cadastrada no momento.</p>
      </div>

      <div v-else class="taxonomy-list">
        <article v-for="item in breeds" :key="item.id" class="taxonomy-item">
          <div class="taxonomy-main">
            <span class="taxonomy-icon">🏷️</span>
            <div>
              <h3>{{ item.name }}</h3>
              <p>{{ item.specie_name || 'Espécie não informada' }}</p>
            </div>
          </div>
        </article>
      </div>
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent="submitBreed" class="modal-form">
          <div class="input-group">
            ...
            <input v-model="form.name" type="text" placeholder="Ex: Nelore" required>
          </div>
