...
from apps.health.animal_biometrics.models import BiometricReading, AnimalAlert


class Command(BaseCommand):
    help = 'Popula banco de dados com dados de teste de biometria'

    def handle(self, *args, **options):
        self.stdout.write("🌱 Iniciando população de dados de biometria...\n")
        
        # Criar leituras de biometria
        reading_count = self.create_biometric_data()
        self.stdout.write(self.style.SUCCESS(f'\n✅ {reading_count} leituras de biometria criadas!'))
        
        # Criar alertas críticos
        alert_count = self.create_critical_alerts()
        self.stdout.write(self.style.SUCCESS(f'✅ {alert_count} alertas críticos criados!'))
        
        self.stdout.write(self.style.SUCCESS("\n🎉 População de dados concluída com sucesso!"))

    def create_biometric_data(self):
        """Cria dados de biometria realistas para os primeiros 5 animais."""
        animals = Animal.objects.all()[:5]
        
        if not animals:
            self.stdout.write(self.style.ERROR("❌ Nenhum animal encontrado. Crie animais primeiro!"))
            return 0
        ...
        for animal in animals:
            ...
            for i in range(10):
                ...
                created_count += 1
                self.stdout.write(f"  ✅ Leitura #{created_count}: {reading.reading_timestamp.strftime('%d/%m/%Y %H:%M')} | "
